...
int main()
{
    ...
    for (i = 0; i < nostate; i++)
    {
       flag= 0;
        for (j = 0; j < nofinal; j++)
        {   // if i is a final state then break
            if (i == finalstate[j])
            {
               flag= 1;
                break;
        ...
        if (flag == 0)
        {  // if i is a non final state then add i to the nonfinalstate array
            nonfinalstate[k++] = i;
        }
    }
    for (i = 0; i < nofinal; i++)
    { // nostate - nofinal = number of non final states
        for (j = 0; j < (nostate - nofinal); j++)
            // marking all the final - nonfinal state combinations as 1
